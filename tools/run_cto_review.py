...
if len(matrix)!=868 or {r["issue_id"] for r in matrix}!={r["issue_id"] for r in issues}:errors.append("Phase G issue coverage drift")
...
if "ADR-001` a `ADR-057" not in status or "Fases A–G:** concluídas" not in status:errors.append("current status stale")
