...
def test_epic_007_integracao() -> None:
...
def test_integration_rejects_failed_or_permissive_automation_report() -> None:
    permissive = {
        "acceptance_evidence": dict.fromkeys(
            integration.EXPECTED_AUTOMATION_AC_IDS, "test_epic_007_automacao"
        ),
        "automation": "EPIC-007_LICENSE_PUBLICATION_GOVERNANCE_CONTROLS",
        "destructive_actions": 0,
        "findings": [],
        "mode": "DRY_RUN",
        "publication_gate": {"decision": "PASS", "failure_mode": "FAIL_CLOSED"},
        "requirement_evidence": {
            requirement: [] for requirement in integration.EXPECTED_REQUIREMENTS
        },
        "status": "PASS",
    }
...
def test_integration_control_plane_has_no_cross_module_import_or_rule_copy() -> None:
    ...
    assert "foundation_validation import" not in text
    assert "control_validation import" not in text
