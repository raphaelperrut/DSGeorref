...
REGISTRY_REL = CONTRACT_ROOT_REL / "runtime-scm-tool-parte-2/schema-compatibility-checkpoint.json"
RUNTIME_EXAMPLE_REL = CONTRACT_ROOT_REL / (
    "runtime-scm-tool-parte-2/examples/runtime-schema-conformance.json"
)
OPENAPI_REL = Path("contracts/http/openapi.yaml")
CANONICAL_CONTRACTS = (
    OPENAPI_REL,
    Path("contracts/events/job-event.schema.json"),
    Path("contracts/artifacts/artifact-set-manifest.schema.json"),
    Path("contracts/domain/processing-plan.schema.json"),
    Path("contracts/domain/quality-report.schema.json"),
    Path("contracts/domain/failure-diagnostic.schema.json"),
)
SENTINEL_REL = Path("unrelated-sentinel.bin")
SENTINEL_BYTES = b"ISSUE-0128 unrelated sentinel\x00\xff\n"
...
def _copy_sources(root: Path) -> None:
    sources = [
        ...
        *(ROOT / TEST_DIR_REL).glob("test_contract_foundation.py"),
        *(ROOT / TEST_DIR_REL).glob("test_runtime_schema_contract.py"),
        *(ROOT / relative for relative in CANONICAL_CONTRACTS),
        ROOT / TASK_REL,
...
def _execute(root: Path, *, dry_run: bool = False) -> Execution:
    command = [sys.executable, "-B", str(root / VALIDATOR_REL), "--repository-root", str(root)]
    if dry_run:
...
def _missing_contract(root: Path) -> None:
    (root / Path("contracts/domain/quality-report.schema.json")).unlink()


def _silent_fallback(root: Path) -> None:
    profile = _read_json(root, RUNTIME_EXAMPLE_REL)
    controls = profile["controls"]
    assert isinstance(controls, dict)
    compatibility = controls["schema_compatibility"]
    assert isinstance(compatibility, dict)
    compatibility["unknown_major"] = "WARN"
    _write_json(root, RUNTIME_EXAMPLE_REL, profile)


def _missing_requirement_test(root: Path) -> None:
    path = root / TEST_DIR_REL / "test_contract_foundation.py"
    source = path.read_text(encoding="utf-8")
    path.write_text(
        source.replace("def test_req_run_008", "def removed_req_run_008"),
        encoding="utf-8",
    )


def _incomplete_registry(root: Path) -> None:
    registry = _read_json(root, REGISTRY_REL)
    entries = registry["entries"]
    assert isinstance(entries, list)
    registry["entries"] = entries[:-1]
    _write_json(root, REGISTRY_REL, registry)


def _invalid_openapi(root: Path) -> None:
    path = root / OPENAPI_REL
    document = yaml.safe_load(path.read_text(encoding="utf-8"))
    del document["components"]["schemas"]["FailureDiagnostic"]
...
def test_epic_004_automacao() -> None:
    with tempfile.TemporaryDirectory(prefix="issue-0128-valid-") as temporary:
        ...
        assert len(report["requirements"]) == 15
    ...
    cases: tuple[tuple[str, str, Mutation], ...] = (
        ...
        ("incomplete_registry", "REGISTRY_INVALID", _incomplete_registry),
        ("invalid_openapi", "OPENAPI_INVALID", _invalid_openapi),
    ...
    for name, code, mutate in cases:
        with tempfile.TemporaryDirectory(prefix=f"issue-0128-{name}-") as temporary:
            sandbox = Path(temporary)
            _copy_sources(sandbox)
            mutate(sandbox)
            before = _snapshot(sandbox)
