# ADR-054 — Observabilidade, telemetria e diagnósticos operacionais
...
- OpenTelemetry é a instrumentação padrão e backends permanecem substituíveis.
- Logs, métricas e traces usam envelope estruturado, baixa cardinalidade e correlação por IDs.
- Readiness, health, resource signals e SLOs são orientados a sintomas.
- Telemetria não é fonte de verdade e não contém secrets ou dados brutos não allowlisted.
...
- **Requisitos owned:** `REQ-MET-001`, `REQ-OBS-001`, `REQ-OBS-002`
- **Épicos relacionados:** `EPIC-005`, `EPIC-039`, `EPIC-077`
