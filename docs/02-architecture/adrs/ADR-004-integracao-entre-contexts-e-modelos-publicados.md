# ADR-004 — Integração entre contexts e modelos publicados

- **Status:** `Accepted`
- **Baseline:** `SAR v2.8 — Fase E`
- **Aprovador:** `Project Owner`
- **Owner normativo:** `ADR-004`
...
- Relações entre contexts seguem o Context Map por Published Language, Open Host Service, ACL, evento ou Process Manager.
- Acesso direto a tabela, ORM model, repository, entidade ou state machine de outro context é proibido.
- Eventos são publicados após commit autoritativo e consumidores são idempotentes.
- Shared kernel de domínio é proibido; somente primitivas técnicas podem ser compartilhadas.
...
- **Requisitos owned:** `REQ-NATIVE-006`
- **Épicos relacionados:** `EPIC-002`, `EPIC-021`, `EPIC-022`, `EPIC-026`
