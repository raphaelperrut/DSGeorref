# ADR-011 — Boundary HTTP com FastAPI, Pydantic e OpenAPI
...
- A API HTTP usa FastAPI, Pydantic e OpenAPI 3.1 como adapter fino sobre application services.
- Modelos de transporte, domínio e persistência são separados por mappers explícitos.
- Cada operação possui request/response específicos, autorização, idempotência, preconditions e exemplos.
- Rotas não acessam diretamente ORM, filesystem, broker ou algoritmos geoespaciais.
...
- **Requisitos owned:** `REQ-TOOL-003`
- **Épicos relacionados:** `EPIC-004`
