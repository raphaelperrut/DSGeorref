# ADR-028 — Contas locais, sessões server-side e Argon2id
...
- **Bounded Contexts:** `BC-002` — Identidade e Controle de Acesso
...
- Contas locais são a baseline; bootstrap administrativo é one-time e não cria senha padrão.
- Sessões Web são opacas, server-side, persistidas, rotacionáveis e revogáveis.
- Passwords usam Argon2id calibrado por profile bounded.
- Recuperação administrativa usa kit offline e cerimônia auditada, sem backdoor.
...
- **Requisitos owned:** `REQ-AUTH-IMPL-002`, `REQ-AUTH-IMPL-004`, `REQ-AUTH-IMPL-006`, `REQ-AUTH-IMPL-008`
- **Épicos relacionados:** `EPIC-008`, `EPIC-009`, `EPIC-010`, `EPIC-011`, `EPIC-041`
