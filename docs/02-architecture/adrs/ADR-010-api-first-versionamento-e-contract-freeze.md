# ADR-010 — API-first, versionamento e contract freeze
...
- Contratos públicos são definidos e congelados antes de implementação paralela de backend, frontend, CLI ou workers.
- Toda operação, evento e schema possui versão, owner, compatibilidade, exemplos, erros e política de evolução.
- Mudança incompatível exige versão explícita, adapter ou ADR substituta.
- OpenAPI, JSON Schemas e catálogos commitados são as fontes normativas; código gerado é derivado.
...
- **Requisitos owned:** `REQ-RUNTIME-005`
- **Épicos relacionados:** `EPIC-012`, `EPIC-013`
...
Este boundary é concretizado por `SPEC-004`. Texto, schemas, exemplos e validadores dessas especificações são obrigatórios. Divergência bloqueia implementação e exige reconciliação pelo Arquiteto.
