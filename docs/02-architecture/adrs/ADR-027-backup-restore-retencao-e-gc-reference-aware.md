# ADR-027 — Backup, restore, retenção e GC reference-aware
...
- **Bounded Contexts:** `BC-013` — Artifacts, Proveniência e Lifecycle, `BC-014` — Operações, Auditoria e Suporte
...
- BackupSet coordena PostgreSQL, manifests, artifacts e checksums.
- Restore drills automatizados e isolados comprovam integridade antes do uso operacional.
- Retenção é por classe, estado, dependência e obrigação; exclusão é auditável.
- GC é reference-aware, idempotente, com dry-run e plano de recomputação.
...
`ADR-018`, `ADR-021`, `ADR-023`, `ADR-025`
...
- **Requisitos owned:** `REQ-EPIC-039`
- **Épicos relacionados:** `EPIC-013`, `EPIC-072`
