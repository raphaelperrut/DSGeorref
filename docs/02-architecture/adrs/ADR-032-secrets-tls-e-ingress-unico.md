# ADR-032 — Secrets, TLS e ingress único
...
- **Bounded Contexts:** `BC-014` — Operações, Auditoria e Suporte, `BC-015` — Release, Instalação e Supply Chain
...
- Existe um único ingress publicado; PostgreSQL, RabbitMQ e roots internos não são expostos diretamente.
- TLS é obrigatório em rede não confiável.
- Secrets usam referências externas e nunca entram em repo, image, logs, manifests, evidence, support bundle ou backup.
- Rotação e revogação são procedimentos versionados e testados.
...
- **Requisitos owned:** `REQ-EPIC-076`, `REQ-LOG-001`, `REQ-NET-001`
- **Épicos relacionados:** `EPIC-008`, `EPIC-039`, `EPIC-041`, `EPIC-076`, `EPIC-078`, `EPIC-079`
