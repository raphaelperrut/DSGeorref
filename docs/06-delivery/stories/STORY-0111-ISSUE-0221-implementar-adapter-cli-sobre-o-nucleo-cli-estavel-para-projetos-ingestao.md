# STORY-0111 / ISSUE-0221 — Implementar adapter CLI sobre o núcleo: CLI estável para projetos, ingestão, jobs e resultados
...
- **TaskEnvelope:** `.codex/tasks/TASK-0111.json`

## História de usuário

Como operador técnico, preciso implementar adapter cli sobre o núcleo para “CLI estável para projetos, ingestão, jobs e resultados”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar adapter CLI sobre o núcleo para a capacidade **CLI estável para projetos, ingestão, jobs e resultados**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/operator_experience/application/cli-estavel-para-projetos-ingestao-jobs-e-resultados/**`
- `src/backend/dsgeorref/contexts/operator_experience/adapters/cli-estavel-para-projetos-ingestao-jobs-e-resultados/**`
## Fora de escopo
...
`REQ-FS1-002`, `REQ-FS1-003`, `REQ-FS1-006`, `REQ-FS1-008`, `REQ-FS1-009`, `REQ-FS1-010`
...
- [ ] O resultado de “CLI estável para projetos, ingestão, jobs e resultados” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-FS1-002, REQ-FS1-003, REQ-FS1-006, REQ-FS1-008, REQ-FS1-009, REQ-FS1-010 possuem evidência explícita no commit candidato.
- [ ] Estados de erro e caminhos fail-closed aplicáveis são testados, sem fallback silencioso.
- [ ] Todos os comandos invocam os mesmos application services usados pela API e workers.
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-020` e o TaskEnvelope `TASK-0111`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0221-01, AC-ISSUE-0221-02, AC-ISSUE-0221-03, AC-ISSUE-0221-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-015`, `ADR-016`, `ADR-023`, `ADR-040`, `ADR-044`, `ADR-045`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-015`, `ADR-016`, `ADR-023`, `ADR-040`, `ADR-044`, `ADR-045`, `ADR-055`
