# STORY-0269 / ISSUE-0379 — Preparar corpus, fixtures e representação tipada: máscara analítica adaptativa, preview/correção, área útil separada da saída e preservação de metadados marginais
...
- **TaskEnvelope:** `.codex/tasks/TASK-0269.json`

## História de usuário

Como operador geoespacial, preciso preparar corpus, fixtures e representação tipada para “máscara analítica adaptativa, preview/correção, área útil separada da saída e preservação de metadados marginais”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Preparar corpus, fixtures e representação tipada para a capacidade **máscara analítica adaptativa, preview/correção, área útil separada da saída e preservação de metadados marginais**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/geo/dsgeorref_geo/contexts/georeferencing/mascara-analitica-adaptativa-preview-correcao-area-uti/**`
## Fora de escopo
...
`REQ-MSK-001`, `REQ-MSK-002`, `REQ-RAS-001`
...
- [ ] Os requisitos REQ-MSK-001, REQ-MSK-002, REQ-RAS-001 possuem evidência explícita no commit candidato.
...
- `test_nodata_alpha_coverage_analysis_mask_separation`
- `test_epic_045_dados`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-007-geoprocessamento.md`, este documento, o épico `EPIC-045` e o TaskEnvelope `TASK-0269`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0379-01, AC-ISSUE-0379-02, AC-ISSUE-0379-03, AC-ISSUE-0379-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-040`, `ADR-041`, `ADR-042`, `ADR-043`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-040`, `ADR-041`, `ADR-042`, `ADR-043`, `ADR-055`
