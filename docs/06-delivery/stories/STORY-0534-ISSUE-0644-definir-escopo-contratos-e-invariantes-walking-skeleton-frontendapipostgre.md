# STORY-0534 / ISSUE-0644 — Definir escopo, contratos e invariantes: walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta
...
- **TaskEnvelope:** `.codex/tasks/TASK-0534.json`

## História de usuário

Como mantenedor de engenharia, preciso definir escopo, contratos e invariantes para “walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Definir escopo, contratos e invariantes para a capacidade **walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `contracts/contexts/engineering_governance/fnd/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
- `docs/02-architecture/design-reviews/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
## Fora de escopo
...
`REQ-EPIC-001`, `REQ-SPRINT-001-004`
...
`STORY-0010`, `STORY-0015`, `STORY-0020`, `STORY-0025`

## Critérios de aceitação

- [ ] O resultado de “walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-EPIC-001, REQ-SPRINT-001-004 possuem evidência explícita no commit candidato.
...
Leia `AGENTS.md`, `.codex/roles/ROLE-002-arquiteto.md`, este documento, o épico `EPIC-086` e o TaskEnvelope `TASK-0534`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0644-01, AC-ISSUE-0644-02, AC-ISSUE-0644-03, AC-ISSUE-0644-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-002`, `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-014`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-002`, `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-014`
