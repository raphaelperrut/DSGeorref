# STORY-0713 / ISSUE-0823 — Slice 2/2 — Implementar domínio e persistência: contas locais, bootstrap único, sessões, tokens e adapter OIDC [REQ-ID]
...
- **TaskEnvelope:** `.codex/tasks/TASK-0713.json`

## História de usuário

Como executor especializado, preciso entregar este slice limitado de requisitos para que a capacidade de **contas locais, bootstrap único, sessões, tokens e adapter OIDC** avance sem mudança ampla ou inferência não documentada.
...
- `src/backend/dsgeorref/contexts/identity_access/application/contas-locais-bootstrap-unico-sessoes-tokens-e-adapter/id-parte-2/**`
- `src/backend/dsgeorref/contexts/identity_access/adapters/contas-locais-bootstrap-unico-sessoes-tokens-e-adapter/id-parte-2/**`
## Fora de escopo
...
- [ ] Cada requisito do slice possui evidência explícita: REQ-ID-001, REQ-ID-002.
...
- `test_network_authentication_local_oidc`
- `test_bootstrap_session_token_recovery`
...
Leia `AGENTS.md`, o papel `.codex/roles/ROLE-004-backend.md`, esta história e `TASK-0713`. Trabalhe somente nos paths permitidos, entregue apenas este slice e interrompa diante de qualquer condição de parada.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0823-01, AC-ISSUE-0823-02, AC-ISSUE-0823-03, AC-ISSUE-0823-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-016`, `ADR-029`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-029`
