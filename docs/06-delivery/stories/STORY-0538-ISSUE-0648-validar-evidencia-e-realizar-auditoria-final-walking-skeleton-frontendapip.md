# STORY-0538 / ISSUE-0648 — Validar evidência e realizar auditoria final: walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta
...
- **TaskEnvelope:** `.codex/tasks/TASK-0538.json`

## História de usuário

Como mantenedor de engenharia, preciso validar evidência e realizar auditoria final para “walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Validar evidência e realizar auditoria final para a capacidade **walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `evidence/reviews/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
## Fora de escopo
...
Leia `AGENTS.md`, `.codex/roles/ROLE-011-reviewer.md`, este documento, o épico `EPIC-086` e o TaskEnvelope `TASK-0538`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0648-01, AC-ISSUE-0648-02, AC-ISSUE-0648-03, AC-ISSUE-0648-04`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-014`
