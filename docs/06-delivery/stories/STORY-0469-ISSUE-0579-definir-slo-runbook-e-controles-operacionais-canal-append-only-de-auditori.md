# STORY-0469 / ISSUE-0579 — Definir SLO, runbook e controles operacionais: canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional
...
- **TaskEnvelope:** `.codex/tasks/TASK-0469.json`

## História de usuário

Como administrador da instância, preciso definir slo, runbook e controles operacionais para “canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Definir SLO, runbook e controles operacionais para a capacidade **canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional**, com saída versionada, testes e evidência no commit candidato.
...
`STORY-0055`, `STORY-0301`, `STORY-0438`
...
- [ ] Os requisitos REQ-AUD-001 possuem evidência explícita no commit candidato.
...
- `test_audit_append_only_digest_and_no_sampling`
- `test_epic_075_contrato`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-009-devops.md`, este documento, o épico `EPIC-075` e o TaskEnvelope `TASK-0469`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0579-01, AC-ISSUE-0579-02, AC-ISSUE-0579-03, AC-ISSUE-0579-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-033`, `ADR-034`, `ADR-055`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-033`, `ADR-034`, `ADR-055`
