# STORY-0536 / ISSUE-0646 — Automatizar validações e controles: walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta
...
- **TaskEnvelope:** `.codex/tasks/TASK-0536.json`

## História de usuário

Como mantenedor de engenharia, preciso automatizar validações e controles para “walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Automatizar validações e controles para a capacidade **walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tests/fnd/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
- `tools/quality/contexts/engineering_governance/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-009-devops.md`, este documento, o épico `EPIC-086` e o TaskEnvelope `TASK-0536`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0646-01, AC-ISSUE-0646-02, AC-ISSUE-0646-03, AC-ISSUE-0646-04`
- **Base de requisitos:** `DERIVED_CONTROL`
- **ADRs governantes:** `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-014`, `ADR-033`, `ADR-034`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-014`, `ADR-033`, `ADR-034`
