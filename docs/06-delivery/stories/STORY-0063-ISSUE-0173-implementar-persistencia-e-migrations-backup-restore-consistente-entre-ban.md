# STORY-0063 / ISSUE-0173 — Implementar persistência e migrations: backup/restore consistente entre banco e arquivos
...
- **TaskEnvelope:** `.codex/tasks/TASK-0063.json`

## História de usuário

Como operador geoespacial, preciso implementar persistência e migrations para “backup/restore consistente entre banco e arquivos”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar persistência e migrations para a capacidade **backup/restore consistente entre banco e arquivos**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/artifact_provenance/application/backup-restore-consistente-entre-banco-e-arquivos/**`
- `src/backend/dsgeorref/contexts/artifact_provenance/adapters/backup-restore-consistente-entre-banco-e-arquivos/**`
## Fora de escopo
...
`REQ-EPIC-039`, `REQ-PRV-001`, `REQ-TOOL-004`
...
- [ ] O resultado de “backup/restore consistente entre banco e arquivos” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-EPIC-039, REQ-PRV-001, REQ-TOOL-004 possuem evidência explícita no commit candidato.
...
- `test_req_epic_039`
- `retention_deletion_restore`
- `test_sqlalchemy_alembic_postgis`
- `test_epic_013_persistencia_roundtrip`
- `test_epic_013_persistencia_recovery`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-013` e o TaskEnvelope `TASK-0063`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0173-01, AC-ISSUE-0173-02, AC-ISSUE-0173-03, AC-ISSUE-0173-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-016`, `ADR-018`, `ADR-027`, `ADR-055`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-026`, `ADR-027`, `ADR-055`
