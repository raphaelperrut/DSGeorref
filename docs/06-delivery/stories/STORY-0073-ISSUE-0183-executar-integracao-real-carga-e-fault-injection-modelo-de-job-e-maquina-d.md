# STORY-0073 / ISSUE-0183 — Executar integração real, carga e fault injection: modelo de job e máquina de estados no PostgreSQL
...
- **Papel executor:** `QA`
- **TaskEnvelope:** `.codex/tasks/TASK-0073.json`

## História de usuário

Como operador geoespacial, preciso executar integração real, carga e fault injection para “modelo de job e máquina de estados no PostgreSQL”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Executar integração real, carga e fault injection para a capacidade **modelo de job e máquina de estados no PostgreSQL**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tests/job/modelo-de-job-e-maquina-de-estados-no-postgresql/**`
- `tools/quality/contexts/job_orchestration/modelo-de-job-e-maquina-de-estados-no-postgresql/**`
- `.github/workflows/modelo-de-job-e-maquina-de-estados-no-postgresql.yaml`
## Fora de escopo
...
`REQ-AIE-003`, `REQ-AIE-007`, `REQ-FS1-005`, `REQ-FS1-007`
...
`STORY-0069`, `STORY-0070`, `STORY-0071`, `STORY-0072`
...
- [ ] Os requisitos REQ-AIE-003, REQ-AIE-007, REQ-FS1-005, REQ-FS1-007 possuem evidência explícita no commit candidato.
...
- `test_epic_014_validacao_happy_path`
- `test_epic_014_validacao_negative_paths`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-008-qa.md`, este documento, o épico `EPIC-014` e o TaskEnvelope `TASK-0073`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0183-01, AC-ISSUE-0183-02, AC-ISSUE-0183-03, AC-ISSUE-0183-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-046`, `ADR-055`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-018`, `ADR-019`, `ADR-020`, `ADR-023`, `ADR-046`, `ADR-055`
