# STORY-0537 / ISSUE-0647 — Integrar a capacidade ao fluxo do repositório: walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta
...
- **TaskEnvelope:** `.codex/tasks/TASK-0537.json`

## História de usuário

Como mantenedor de engenharia, preciso integrar a capacidade ao fluxo do repositório para “walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Integrar a capacidade ao fluxo do repositório para a capacidade **walking skeleton frontend→API→PostgreSQL→RabbitMQ/Celery→worker→artefato diagnóstico, executável e testado ponta a ponta**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tools/governance/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
- `docs/03-engineering/contexts/engineering_governance/walking-skeleton-frontendapipostgresqlrabbitmq-celeryw/**`
## Fora de escopo
...
Leia `AGENTS.md`, `.codex/roles/ROLE-003-tech-lead.md`, este documento, o épico `EPIC-086` e o TaskEnvelope `TASK-0537`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0647-01, AC-ISSUE-0647-02, AC-ISSUE-0647-03, AC-ISSUE-0647-04`
