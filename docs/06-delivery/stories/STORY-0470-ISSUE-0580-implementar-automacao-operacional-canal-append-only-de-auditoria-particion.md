# STORY-0470 / ISSUE-0580 — Implementar automação operacional: canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional
...
- **TaskEnvelope:** `.codex/tasks/TASK-0470.json`

## História de usuário

Como administrador da instância, preciso implementar automação operacional para “canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar automação operacional para a capacidade **canal append-only de auditoria, particionamento, digests, consulta e exportação assinada opcional**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tests/ops/contexts/operations_audit/canal-append-only-de-auditoria-particionamento-digests/**`
- `tools/quality/contexts/operations_audit/canal-append-only-de-auditoria-particionamento-digests/**`
- `.github/workflows/canal-append-only-de-auditoria-particionamento-digests.yaml`
## Fora de escopo
...
`REQ-INS-004`, `REQ-RMQ-004`, `REQ-SMO-004`, `REQ-UPG-003`
...
- [ ] Os requisitos REQ-INS-004, REQ-RMQ-004, REQ-SMO-004, REQ-UPG-003 possuem evidência explícita no commit candidato.
...
- `test_sanitized_local_support_bundle_no_exfiltration_and_repair_dry_run`
- `test_quota_pinning_dependency_aware_eviction_dry_run_and_recompute_plan`
- `test_role_based_scheduler_diagnostics_safe_controls_and_append_only_audit`
- `test_post_migration_multidimensional_evidence_gate_canary_and_atomic_cutover`
- `test_epic_075_implementacao`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-009-devops.md`, este documento, o épico `EPIC-075` e o TaskEnvelope `TASK-0470`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0580-01, AC-ISSUE-0580-02, AC-ISSUE-0580-03, AC-ISSUE-0580-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-030`, `ADR-033`, `ADR-034`, `ADR-035`, `ADR-039`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-030`, `ADR-033`, `ADR-034`, `ADR-035`, `ADR-039`
...
- **Resultado:** `PASS`
- **Risk tier:** `CRITICAL`
- **Controles aplicáveis:** `CTO-001, CTO-002, CTO-003, CTO-004, CTO-005, CTO-006, CTO-008, CTO-009, CTO-010, CTO-012, CTO-013, CTO-014, CTO-015`
