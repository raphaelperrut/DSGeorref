# STORY-0464 / ISSUE-0574 — Implementar persistência e migrations: GC reference-aware, tombstone, quarentena, período de graça e reconciliação
...
- **TaskEnvelope:** `.codex/tasks/TASK-0464.json`

## História de usuário

Como operador geoespacial, preciso implementar persistência e migrations para “GC reference-aware, tombstone, quarentena, período de graça e reconciliação”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar persistência e migrations para a capacidade **GC reference-aware, tombstone, quarentena, período de graça e reconciliação**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/artifact_provenance/application/gc-reference-aware-tombstone-quarentena-periodo-de-gra/**`
- `src/backend/dsgeorref/contexts/artifact_provenance/adapters/gc-reference-aware-tombstone-quarentena-periodo-de-gra/**`
## Fora de escopo
...
- [ ] O resultado de “GC reference-aware, tombstone, quarentena, período de graça e reconciliação” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-GC-001 possuem evidência explícita no commit candidato.
...
- `test_reference_aware_gc_tombstone_quarantine_race_idempotency`
- `test_epic_074_persistencia_roundtrip`
- `test_epic_074_persistencia_recovery`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-074` e o TaskEnvelope `TASK-0464`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0574-01, AC-ISSUE-0574-02, AC-ISSUE-0574-03, AC-ISSUE-0574-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-016`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-016`, `ADR-018`, `ADR-019`
