# STORY-0232 / ISSUE-0342 — Definir SLO, runbook e controles operacionais: instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers
...
- **TaskEnvelope:** `.codex/tasks/TASK-0232.json`

## História de usuário

Como administrador da instância, preciso definir slo, runbook e controles operacionais para “instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Definir SLO, runbook e controles operacionais para a capacidade **instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tests/ops/contexts/operations_audit/instrumentacao-opentelemetry-correlation-ids-metricas/**`
- `tools/quality/contexts/operations_audit/instrumentacao-opentelemetry-correlation-ids-metricas/**`
- `.github/workflows/instrumentacao-opentelemetry-correlation-ids-metricas.yaml`
## Fora de escopo
...
`REQ-AIE-001`, `REQ-AIE-005`, `REQ-OBS-002`, `REQ-RUNTIME-010`, `REQ-WORKER-001`, `REQ-WORKER-010`
...
`STORY-0025`, `STORY-0081`, `STORY-0109`, `STORY-0424`, `STORY-0515`

## Critérios de aceitação

- [ ] O resultado de “instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-AIE-001, REQ-AIE-005, REQ-OBS-002, REQ-RUNTIME-010, REQ-WORKER-001, REQ-WORKER-010 possuem evidência explícita no commit candidato.
...
- `test_runtime_decision_10`
- `test_epic_039_contrato`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-009-devops.md`, este documento, o épico `EPIC-039` e o TaskEnvelope `TASK-0232`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0342-01, AC-ISSUE-0342-02, AC-ISSUE-0342-03, AC-ISSUE-0342-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-017`, `ADR-033`, `ADR-034`, `ADR-038`, `ADR-051`, `ADR-054`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-017`, `ADR-033`, `ADR-034`, `ADR-038`, `ADR-051`, `ADR-052`, `ADR-053`, `ADR-054`
...
|---|---|---|
| Dependências | 5 predecessores explícitos | PASS |
