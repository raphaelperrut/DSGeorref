# STORY-0070 / ISSUE-0180 — Implementar runner, worker ou scheduler: modelo de job e máquina de estados no PostgreSQL
...
- **TaskEnvelope:** `.codex/tasks/TASK-0070.json`

## História de usuário

Como operador geoespacial, preciso implementar runner, worker ou scheduler para “modelo de job e máquina de estados no PostgreSQL”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar runner, worker ou scheduler para a capacidade **modelo de job e máquina de estados no PostgreSQL**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/job_orchestration/application/modelo-de-job-e-maquina-de-estados-no-postgresql/**`
- `src/backend/dsgeorref/contexts/job_orchestration/adapters/modelo-de-job-e-maquina-de-estados-no-postgresql/**`
## Fora de escopo
...
`REQ-EPIC-014`, `REQ-WORKER-002`, `REQ-WORKER-004`, `REQ-WORKER-005`, `REQ-WORKER-006`, `REQ-WORKER-007`, `REQ-WORKER-008`, `REQ-WORKER-009`
...
- [ ] O resultado de “modelo de job e máquina de estados no PostgreSQL” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-EPIC-014, REQ-WORKER-002, REQ-WORKER-004, REQ-WORKER-005, REQ-WORKER-006, REQ-WORKER-007 possuem evidência explícita no commit candidato.
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-014` e o TaskEnvelope `TASK-0070`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0180-01, AC-ISSUE-0180-02, AC-ISSUE-0180-03, AC-ISSUE-0180-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-018`, `ADR-020`, `ADR-037`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-020`, `ADR-023`, `ADR-037`
