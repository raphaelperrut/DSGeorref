# STORY-0358 / ISSUE-0468 — Definir contrato científico e invariantes: gateway por capacidades, adapters STAC/API oficial, testes de contrato, allowlist, SSRF/egress e estados de licença/disponibilidade
...
- **TaskEnvelope:** `.codex/tasks/TASK-0358.json`

## História de usuário

Como operador geoespacial, preciso definir contrato científico e invariantes para “gateway por capacidades, adapters STAC/API oficial, testes de contrato, allowlist, SSRF/egress e estados de licença/disponibilidade”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Definir contrato científico e invariantes para a capacidade **gateway por capacidades, adapters STAC/API oficial, testes de contrato, allowlist, SSRF/egress e estados de licença/disponibilidade**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `contracts/contexts/reference_discovery/geo/gateway-por-capacidades-adapters-stac-api-oficial-test/**`
- `docs/02-architecture/design-reviews/gateway-por-capacidades-adapters-stac-api-oficial-test/**`
## Fora de escopo
...
`REQ-CRS-005`, `REQ-CRS-007`, `REQ-PUB-003`, `REQ-SRC-003`
...
`STORY-0163`, `STORY-0020`, `STORY-0249`

## Critérios de aceitação

- [ ] O resultado de “gateway por capacidades, adapters STAC/API oficial, testes de contrato, allowlist, SSRF/egress e estados de licença/disponibilidade” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-CRS-005, REQ-CRS-007, REQ-PUB-003, REQ-SRC-003 possuem evidência explícita no commit candidato.
...
- `test_reference_crs_fail_closed_metadata_area_operation_roundtrip`
- `test_web_service_capability_crs_negotiation_and_actual_response_validation`
- `test_provider_policy_manifest_asset_license_record_expiry_attribution_and_fail_closed_acquisition`
- `test_provider_capability_contract_allowlist_ssrf_no_purchase`
- `test_epic_058_contrato`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-002-arquiteto.md`, este documento, o épico `EPIC-058` e o TaskEnvelope `TASK-0358`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0468-01, AC-ISSUE-0468-02, AC-ISSUE-0468-03, AC-ISSUE-0468-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-034`, `ADR-041`, `ADR-047`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-012`, `ADR-034`, `ADR-041`, `ADR-042`, `ADR-043`, `ADR-047`, `ADR-049`
