# STORY-0097 / ISSUE-0207 — Implementar modelo e application services: batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens
...
- **TaskEnvelope:** `.codex/tasks/TASK-0097.json`

## História de usuário

Como operador geoespacial, preciso implementar modelo e application services para “batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar modelo e application services para a capacidade **batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/job_orchestration/application/batch-hierarquico-chunking-backpressure-checkpoints-e/**`
- `src/backend/dsgeorref/contexts/job_orchestration/adapters/batch-hierarquico-chunking-backpressure-checkpoints-e/**`
## Fora de escopo
...
`REQ-BEX-002`, `REQ-BEX-004`, `REQ-BEX-006`, `REQ-BEX-010`, `REQ-MOS-001`
...
- [ ] O resultado de “batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-BEX-002, REQ-BEX-004, REQ-BEX-006, REQ-BEX-010, REQ-MOS-001 possuem evidência explícita no commit candidato.
- [ ] Estados de erro e caminhos fail-closed aplicáveis são testados, sem fallback silencioso.
- [ ] Transições inválidas são rejeitadas e invariantes persistem sob concorrência.
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-018` e o TaskEnvelope `TASK-0097`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0207-01, AC-ISSUE-0207-02, AC-ISSUE-0207-03, AC-ISSUE-0207-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-038`, `ADR-039`, `ADR-049`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-038`, `ADR-039`, `ADR-049`
