# STORY-0105 / ISSUE-0215 — Implementar runner, worker ou scheduler: Resource Governor, admission control e orçamento adaptativo de RAM/CPU/GPU/disco
...
- **TaskEnvelope:** `.codex/tasks/TASK-0105.json`

## História de usuário

Como operador geoespacial, preciso implementar runner, worker ou scheduler para “Resource Governor, admission control e orçamento adaptativo de RAM/CPU/GPU/disco”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar runner, worker ou scheduler para a capacidade **Resource Governor, admission control e orçamento adaptativo de RAM/CPU/GPU/disco**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/job_orchestration/application/resource-governor-admission-control-e-orcamento-adapta/**`
- `src/backend/dsgeorref/contexts/job_orchestration/adapters/resource-governor-admission-control-e-orcamento-adapta/**`
## Fora de escopo
...
`REQ-BEX-007`, `REQ-BEX-008`, `REQ-BEX-009`, `REQ-DIS-002`, `REQ-RES-001`, `REQ-SCH-001`
...
- [ ] O resultado de “Resource Governor, admission control e orçamento adaptativo de RAM/CPU/GPU/disco” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-BEX-007, REQ-BEX-008, REQ-BEX-009, REQ-DIS-002, REQ-RES-001, REQ-SCH-001 possuem evidência explícita no commit candidato.
...
- `test_budgeted_neighbor_retrieval_recall_and_unexamined_state`
- `test_epic_019_execucao`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-019` e o TaskEnvelope `TASK-0105`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0215-01, AC-ISSUE-0215-02, AC-ISSUE-0215-03, AC-ISSUE-0215-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-037`, `ADR-038`, `ADR-039`, `ADR-040`, `ADR-047`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-012`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-037`, `ADR-038`, `ADR-039`, `ADR-040`, `ADR-047`
