# STORY-0297 / ISSUE-0407 — Implementar persistência e migrations: lineage normalizado, manifestos e bundle de reprodução sob demanda
...
- **TaskEnvelope:** `.codex/tasks/TASK-0297.json`

## História de usuário

Como operador geoespacial, preciso implementar persistência e migrations para “lineage normalizado, manifestos e bundle de reprodução sob demanda”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Implementar persistência e migrations para a capacidade **lineage normalizado, manifestos e bundle de reprodução sob demanda**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/backend/dsgeorref/contexts/artifact_provenance/application/lineage-normalizado-manifestos-e-bundle-de-reproducao/**`
- `src/backend/dsgeorref/contexts/artifact_provenance/adapters/lineage-normalizado-manifestos-e-bundle-de-reproducao/**`
## Fora de escopo
...
`REQ-BKP-001`, `REQ-CAC-001`, `REQ-RMQ-002`, `REQ-SDR-004`, `REQ-SRG-004`
...
- [ ] O resultado de “lineage normalizado, manifestos e bundle de reprodução sob demanda” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-BKP-001, REQ-CAC-001, REQ-RMQ-002, REQ-SDR-004, REQ-SRG-004 possuem evidência explícita no commit candidato.
- [ ] Estados de erro e caminhos fail-closed aplicáveis são testados, sem fallback silencioso.
- [ ] Migration forward e rollback/recovery aplicável são demonstrados com dados de teste.

## Testes obrigatórios

- `test_backupset_consistency_manifest_external_roots`
- `test_content_addressed_cache_license_retention_lineage_gc`
- `test_persistence_classes_recomputability_lineage_retention_and_resume`
- `test_scientific_reproducibility_record_replay_levels_promotion_matrix_and_retention`
- `test_epic_049_persistencia_roundtrip`
- `test_epic_049_persistencia_recovery`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-004-backend.md`, este documento, o épico `EPIC-049` e o TaskEnvelope `TASK-0297`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0407-01, AC-ISSUE-0407-02, AC-ISSUE-0407-03, AC-ISSUE-0407-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-016`, `ADR-018`, `ADR-047`, `ADR-053`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-016`, `ADR-018`, `ADR-019`, `ADR-025`, `ADR-047`, `ADR-052`, `ADR-053`
