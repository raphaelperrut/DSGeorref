# STORY-0599 / ISSUE-0709 — Preparar corpus, fixtures e representação tipada: Grade, resolução e reamostragem
...
- **TaskEnvelope:** `.codex/tasks/TASK-0599.json`

## História de usuário

Como operador geoespacial, preciso preparar corpus, fixtures e representação tipada para “Grade, resolução e reamostragem”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Preparar corpus, fixtures e representação tipada para a capacidade **Grade, resolução e reamostragem**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `src/geo/dsgeorref_geo/contexts/georeferencing/grade-resolucao-e-reamostragem/**`
## Fora de escopo
...
`REQ-RAS-002`, `REQ-RAS-003`, `REQ-RAS-004`, `REQ-RAS-005`
...
- [ ] Os requisitos REQ-RAS-002, REQ-RAS-003, REQ-RAS-004, REQ-RAS-005 possuem evidência explícita no commit candidato.
...
- `test_output_grid_extent_alignment_pixel_anchor_and_no_half_pixel_shift`
- `test_resolution_native_information_jacobian_limits_and_no_false_superresolution`
- `test_resampling_kernel_by_band_content_scale_and_mask_integrity`
- `test_cog_grid_metadata_overviews_validity_roundtrip_and_atomic_publication`
- `test_epic_097_dados`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-007-geoprocessamento.md`, este documento, o épico `EPIC-097` e o TaskEnvelope `TASK-0599`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0709-01, AC-ISSUE-0709-02, AC-ISSUE-0709-03, AC-ISSUE-0709-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-040`, `ADR-041`, `ADR-042`, `ADR-043`, `ADR-049`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-040`, `ADR-041`, `ADR-042`, `ADR-043`, `ADR-049`
