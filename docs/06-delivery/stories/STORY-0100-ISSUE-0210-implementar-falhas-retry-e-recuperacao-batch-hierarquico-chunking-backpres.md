# STORY-0100 / ISSUE-0210 — Automatizar testes de resiliência, retry e recuperação: batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens
...
- **TaskEnvelope:** `.codex/tasks/TASK-0100.json`

## História de usuário

Como operador geoespacial, preciso automatizar testes de resiliência, retry e recuperação para “batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Automatizar testes de resiliência, retry e recuperação para a capacidade **batch hierárquico, chunking, backpressure, checkpoints e retomada para lotes usuais de 40–300 imagens**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `tests/job/batch-hierarquico-chunking-backpressure-checkpoints-e/**`
- `tools/quality/contexts/job_orchestration/batch-hierarquico-chunking-backpressure-checkpoints-e/**`
- `.github/workflows/batch-hierarquico-chunking-backpressure-checkpoints-e.yaml`
## Fora de escopo
...
- [ ] Retry, timeout, cancelamento, drain e recovery aplicáveis preservam consistência.

## Testes obrigatórios

- `test_epic_018_resiliencia`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-009-devops.md`, este documento, o épico `EPIC-018` e o TaskEnvelope `TASK-0100`. Trabalhe somente nos paths permitidos. Entregue o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0210-01, AC-ISSUE-0210-02, AC-ISSUE-0210-03, AC-ISSUE-0210-04`
- **Base de requisitos:** `DERIVED_CONTROL`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-033`, `ADR-034`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-033`, `ADR-034`
...
- **Resultado:** `PASS`
- **Risk tier:** `CRITICAL`
- **Controles aplicáveis:** `CTO-001, CTO-002, CTO-003, CTO-004, CTO-005, CTO-006, CTO-008, CTO-009, CTO-012, CTO-013, CTO-015`
