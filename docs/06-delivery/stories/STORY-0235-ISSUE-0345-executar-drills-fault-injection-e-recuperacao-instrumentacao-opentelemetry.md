# STORY-0235 / ISSUE-0345 — Executar drills, fault injection e recuperação: instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers
...
- **Papel executor:** `QA`
- **TaskEnvelope:** `.codex/tasks/TASK-0235.json`

## História de usuário

Como administrador da instância, preciso executar drills, fault injection e recuperação para “instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Executar drills, fault injection e recuperação para a capacidade **instrumentação OpenTelemetry, correlation IDs, métricas, dashboards e alertas para API, RabbitMQ e workers**, com saída versionada, testes e evidência no commit candidato.
...
- [ ] Drill produz evidência de detecção, contenção, recuperação e impacto residual.

## Testes obrigatórios

- `test_epic_039_falhas_happy_path`
- `test_epic_039_falhas_negative_paths`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-008-qa.md`, este documento, o épico `EPIC-039` e o TaskEnvelope `TASK-0235`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0345-01, AC-ISSUE-0345-02, AC-ISSUE-0345-03, AC-ISSUE-0345-04`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-012`
