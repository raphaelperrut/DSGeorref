# STORY-0488 / ISSUE-0598 — Definir políticas e contratos fail-closed: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
...
- **TaskEnvelope:** `.codex/tasks/TASK-0488.json`

## História de usuário

Como administrador de segurança, preciso definir políticas e contratos fail-closed para “contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault”, para que o resultado do épico seja implementável, verificável e integrável sem violar seus contratos.

## Resultado verificável

Definir políticas e contratos fail-closed para a capacidade **contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault**, com saída versionada, testes e evidência no commit candidato.

## Escopo

- `contracts/contexts/operations_audit/sec/contrato-de-secrets-setup-permissoes-rotacao-recuperac/**`
- `docs/02-architecture/design-reviews/contrato-de-secrets-setup-permissoes-rotacao-recuperac/**`
## Fora de escopo
...
`REQ-ARTLAYOUT-005`, `REQ-ARTLAYOUT-006`, `REQ-ARTLAYOUT-007`, `REQ-RUNTIME-004`
...
- [ ] O resultado de “contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault” é observável por contrato, interface, artifact ou evidência executável.
- [ ] Os requisitos REQ-ARTLAYOUT-005, REQ-ARTLAYOUT-006, REQ-ARTLAYOUT-007, REQ-RUNTIME-004 possuem evidência explícita no commit candidato.
...
- `test_req_artlayout_005`
- `test_req_artlayout_006`
- `test_req_artlayout_007`
- `test_runtime_decision_4`
- `test_epic_078_contrato`
...
Leia `AGENTS.md`, `.codex/roles/ROLE-002-arquiteto.md`, este documento, o épico `EPIC-078` e o TaskEnvelope `TASK-0488`. Trabalhe somente nos paths permitidos. Implemente o menor incremento que satisfaça todos os critérios, execute os testes, registre evidências e interrompa quando qualquer condição de parada ocorrer.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0598-01, AC-ISSUE-0598-02, AC-ISSUE-0598-03, AC-ISSUE-0598-04`
- **Base de requisitos:** `MIXED`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-023`, `ADR-024`, `ADR-040`, `ADR-045`
- **Conflito:** `Nenhum`
...
- **Resultado:** `PASS`
- **ADRs aplicáveis:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-016`, `ADR-023`, `ADR-024`, `ADR-040`, `ADR-045`
