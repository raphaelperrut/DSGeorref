# ISSUE-0078 — contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
...
Como equipe de entrega do DSGeorref, precisamos contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault, para que a capacidade de produto vinculada seja entregue com qualidade verificável e integração controlada.

## Critérios de aceitação

- [ ] O resultado de produto de `EPIC-078` é demonstravelmente satisfeito.
...
- `STORY-0487` / `ISSUE-0597` — `Security` — Modelar ameaças e requisitos de controle: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
- `STORY-0488` / `ISSUE-0598` — `Arquiteto` — Definir políticas e contratos fail-closed: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
- `STORY-0489` / `ISSUE-0599` — `Backend` — Implementar controles e enforcement: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
- `STORY-0490` / `ISSUE-0600` — `Security` — Executar testes negativos e ofensivos: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
- `STORY-0491` / `ISSUE-0601` — `DevOps` — Instrumentar detecção, resposta e runbook: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
- `STORY-0492` / `ISSUE-0602` — `Reviewer` — Auditar evidência de segurança final: contrato de secrets, setup, permissões, rotação, recuperação e adapters opcionais de vault
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0078-01, AC-ISSUE-0078-02, AC-ISSUE-0078-03, AC-ISSUE-0078-04, AC-ISSUE-0078-05, AC-ISSUE-0078-06`
- **Requisitos do épico:** `5`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-016`, `ADR-023`, `ADR-024`, `ADR-030`, `ADR-031`, `ADR-032`, `ADR-033`, `ADR-034`, `ADR-040`, `ADR-045`
- **Conflito:** `Nenhum`
