# ISSUE-0003 — monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
...
Como equipe de entrega do DSGeorref, precisamos monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis, para que a capacidade de produto vinculada seja entregue com qualidade verificável e integração controlada.

## Critérios de aceitação

- [ ] O resultado de produto de `EPIC-003` é demonstravelmente satisfeito.
...
- `STORY-0011` / `ISSUE-0121` — `Arquiteto` — Definir escopo, contratos e invariantes: monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
- `STORY-0012` / `ISSUE-0122` — `Tech Lead` — Materializar a fundação executável: monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
- `STORY-0013` / `ISSUE-0123` — `DevOps` — Automatizar validações e controles: monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
- `STORY-0014` / `ISSUE-0124` — `Tech Lead` — Integrar a capacidade ao fluxo do repositório: monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
- `STORY-0015` / `ISSUE-0125` — `Reviewer` — Validar evidência e realizar auditoria final: monorepo greenfield com CLI/API/Web mínimos e checks reproduzíveis
...
- [ ] Layout por domínio/capability/camada está congelado.
- [ ] Fitness functions bloqueiam ciclos, módulos genéricos e arquivos Python acima do limite hard.
- [ ] API, CLI e worker reutilizam application services.
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0003-01, AC-ISSUE-0003-02, AC-ISSUE-0003-03, AC-ISSUE-0003-04, AC-ISSUE-0003-05, AC-ISSUE-0003-06`
- **Requisitos do épico:** `3`
- **ADRs governantes:** `ADR-001`, `ADR-003`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-014`, `ADR-033`, `ADR-034`
- **Conflito:** `Nenhum`
