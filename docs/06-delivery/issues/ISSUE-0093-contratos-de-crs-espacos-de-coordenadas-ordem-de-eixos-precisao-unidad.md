# ISSUE-0093 — contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
...
Como equipe de entrega do DSGeorref, precisamos contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster, para que a capacidade de produto vinculada seja entregue com qualidade verificável e integração controlada.

## Critérios de aceitação

- [ ] O resultado de produto de `EPIC-093` é demonstravelmente satisfeito.
...
- `STORY-0570` / `ISSUE-0680` — `Arquiteto` — Definir contrato científico e invariantes: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0571` / `ISSUE-0681` — `Geoprocessamento` — Preparar corpus, fixtures e representação tipada: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0572` / `ISSUE-0682` — `Geoprocessamento` — Implementar o núcleo algorítmico: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0573` / `ISSUE-0683` — `Geoprocessamento` — Integrar ao ProcessingPlan e pipeline: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0574` / `ISSUE-0684` — `Geoprocessamento` — Produzir métricas, diagnóstico e lineage: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0575` / `ISSUE-0685` — `QA` — Executar benchmark, negativos e regressão científica: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
- `STORY-0576` / `ISSUE-0686` — `Reviewer` — Auditar evidência científica e final: contratos de CRS, espaços de coordenadas, ordem de eixos, precisão/unidades e validade raster
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0093-01, AC-ISSUE-0093-02, AC-ISSUE-0093-03, AC-ISSUE-0093-04, AC-ISSUE-0093-05, AC-ISSUE-0093-06`
- **Requisitos do épico:** `5`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-010`, `ADR-011`, `ADR-041`, `ADR-042`, `ADR-043`
- **Conflito:** `Nenhum`
