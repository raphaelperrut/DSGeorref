# ISSUE-0081 — orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
...
Como equipe de entrega do DSGeorref, precisamos orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore, para que a capacidade de produto vinculada seja entregue com qualidade verificável e integração controlada.

## Critérios de aceitação

- [ ] O resultado de produto de `EPIC-081` é demonstravelmente satisfeito.
...
- `STORY-0505` / `ISSUE-0615` — `Tech Lead` — Definir gate, versão e critérios de release: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
- `STORY-0506` / `ISSUE-0616` — `DevOps` — Implementar pipeline e artifacts de release: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
- `STORY-0507` / `ISSUE-0617` — `DevOps` — Implementar upgrade, rollback e compatibilidade: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
- `STORY-0508` / `ISSUE-0618` — `Security` — Gerar evidências, SBOM e attestations: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
- `STORY-0509` / `ISSUE-0619` — `QA` — Executar instalação limpa e rehearsal: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
- `STORY-0510` / `ISSUE-0620` — `Reviewer` — Auditar release candidata final: orquestrador de upgrade, preflight, migrations explícitas, health/smoke tests e rollback/restore
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0081-01, AC-ISSUE-0081-02, AC-ISSUE-0081-03, AC-ISSUE-0081-04, AC-ISSUE-0081-05, AC-ISSUE-0081-06`
- **Requisitos do épico:** `2`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-016`, `ADR-030`, `ADR-031`, `ADR-032`, `ADR-033`, `ADR-034`, `ADR-035`
- **Conflito:** `Nenhum`
