# ISSUE-0079 — ingress único, TLS, redes privadas, limites e testes de exposição de portas
...
Como equipe de entrega do DSGeorref, precisamos ingress único, TLS, redes privadas, limites e testes de exposição de portas, para que a capacidade de produto vinculada seja entregue com qualidade verificável e integração controlada.

## Critérios de aceitação

- [ ] O resultado de produto de `EPIC-079` é demonstravelmente satisfeito.
...
- `STORY-0493` / `ISSUE-0603` — `DevOps` — Definir SLO, runbook e controles operacionais: ingress único, TLS, redes privadas, limites e testes de exposição de portas
- `STORY-0494` / `ISSUE-0604` — `DevOps` — Implementar automação operacional: ingress único, TLS, redes privadas, limites e testes de exposição de portas
- `STORY-0495` / `ISSUE-0605` — `DevOps` — Instrumentar sinais, dashboards e alertas: ingress único, TLS, redes privadas, limites e testes de exposição de portas
- `STORY-0496` / `ISSUE-0606` — `QA` — Executar drills, fault injection e recuperação: ingress único, TLS, redes privadas, limites e testes de exposição de portas
- `STORY-0497` / `ISSUE-0607` — `Security` — Validar acesso, redaction e exposição: ingress único, TLS, redes privadas, limites e testes de exposição de portas
- `STORY-0498` / `ISSUE-0608` — `Reviewer` — Auditar evidência operacional final: ingress único, TLS, redes privadas, limites e testes de exposição de portas
...
- **Resultado:** `PASS`
- **Critérios rastreados:** `AC-ISSUE-0079-01, AC-ISSUE-0079-02, AC-ISSUE-0079-03, AC-ISSUE-0079-04, AC-ISSUE-0079-05, AC-ISSUE-0079-06`
- **Requisitos do épico:** `1`
- **ADRs governantes:** `ADR-003`, `ADR-004`, `ADR-005`, `ADR-006`, `ADR-007`, `ADR-008`, `ADR-009`, `ADR-010`, `ADR-011`, `ADR-030`, `ADR-031`, `ADR-032`, `ADR-033`, `ADR-034`
- **Conflito:** `Nenhum`
